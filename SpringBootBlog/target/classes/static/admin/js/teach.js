...
function subArticle(status) {
    var title = $('#teachForm input[name=title]').val();
    ...
    $("#teachForm #status").val(status);
    $("#teachForm #categories").val($('#multiple-sel').val());
    $("#teachForm #type").val($('#multiple-type').val());
    var params = $("#teachForm").serialize();
    console.log(params);
    var url = $('#teachForm #tid').val() != '' ? '/admin/teach/modify' : '/admin/teach/publish';
    tale.post({
        url:url,
        data:params,
        success: function (result) {
            if (result && result.code == 'success') {
                tale.alertOk({
                    ...
                    then: function () {
                        setTimeout(function () {
                            window.location.href = '/admin/teach';
                        }, 500);
                    }
                });
            } else {
